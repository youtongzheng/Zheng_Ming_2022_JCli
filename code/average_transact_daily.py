...
# Parse inputs
if len(sys.argv) !=5:
    print('ERROR---Usage: python average_transact_daily.py ' +
        '<year> <hemisphere> <dir>' )
    exit()
...
#am4 nh
f = xr.open_mfdataset(glob.glob(path + hemisphere + '_' + year + "_daily_transect_" + exp +".nc"),decode_cf = True)
# f = f.sel(time=slice("2006-06-12", "2014-12-31"))

f['v10_ave'] = f.v_ref.sel(distance = slice(-3.5, 3.5)).mean(dim = 'distance')
f['u10_ave'] = f.u_ref.sel(distance = slice(-3.5, 3.5)).mean(dim = 'distance')
