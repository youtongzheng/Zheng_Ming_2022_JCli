...
f['vas_ave'] = f.vas.sel(distance = slice(-3.5, 3.5)).mean(dim = 'distance')
f['uas_ave'] = f.uas.sel(distance = slice(-3.5, 3.5)).mean(dim = 'distance')

if hemisphere[0:2] == 'nh':
    fpos = f.where(f.vas_ave >= 3, drop = True)
    fpos_weaku = f.where((f.vas_ave >= 3) & (f.uas_ave >= -2) & (f.uas_ave <= 2), drop = True)
    fpos_strongu = f.where((f.vas_ave >= 3) & ((f.uas_ave < -2) | (f.uas_ave > 2)), drop = True)

    fneg = f.where(f.vas_ave < -3, drop = True)
    fneg_weaku = f.where((f.vas_ave < -3) & (f.uas_ave >= -2) & (f.uas_ave <= 2), drop = True)
    fneg_strongu = f.where((f.vas_ave < -3) & ((f.uas_ave < -2) | (f.uas_ave > 2)), drop = True)
else:
    fpos = f.where(f.vas_ave <= -3, drop = True)
    fpos_weaku = f.where((f.vas_ave <= -3) & (f.uas_ave >= -2) & (f.uas_ave <= 2), drop = True)
    fpos_strongu = f.where((f.vas_ave <= -3) & ((f.uas_ave < -2) | (f.uas_ave > 2)), drop = True)
